...
function getOtherAcs(team) {
  ...
  return R.compose(
    R.fromPairs,
    R.map(x => [x[0], R.pluck('username', x[1])]),
    R.filter(u => u[1]),
    zipAcs
  )(team);
}
...
const transformDate = R.curry((unit, date) =>
  moment(date).utc().startOf(unit).toISOString());

// data -> [names]
const getAllUserNames = R.compose(R.uniq, R.flatten, R.map(R.keys), R.values);

// name -> data -> [{edits}]
const getEditsByName = name =>
  R.compose(sumEditObjs, R.filter(R.identity), R.pluck(name), R.values);
...
const getEditsByAllUsers = data =>
  R.zipObj(usernames, R.juxt(usernames.map(getEditsByName))(data));
...
const getEditsByTime = unit =>
  R.pipe(
    R.toPairs,
    R.map(([date, edits]) =>
      R.fromPairs([[transformDate(unit)(date), R.values(edits)]])),
...
function sumEditObjs(editArray) {
  ...
  const addSameObjs = R.curry((base, data) =>
    R.reduce(R.mergeWith(R.add), base)(data));
  ...
  const get2ndLevelTags = R.compose(
    R.sum,
    R.flatten,
    R.map(R.values),
    R.values
...
function getEditsByUsersByTime(unit, data) {
  ...
  const getEditsByTime = unit => {
    return R.compose(
      removeUnwantedData,
      groupByTime,
      R.map(d => [transformDate(unit)(d[0]), d[1]]),
      R.toPairs
    );
  };
  ...
  const applySumEditsByTimeToUser = R.curry((username, data) =>
    sumEditsByTime(unit, pluckUsers(username, data)));

  const applySumEditsByTimeToAllUsers = R.map(
    applySumEditsByTimeToUser,
    usernames
  );

  return R.zipObj(usernames, R.juxt(applySumEditsByTimeToAllUsers)(data));
}
...
export default class Data {
  constructor(data) {
    ...
    this._dataTeam = R.zipObj(
      R.pluck('username', this._dataTeam),
      this._dataTeam
    );
